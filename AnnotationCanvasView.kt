...
class AnnotationCanvasView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isDrawing && currentPath.isNotEmpty()) {
            drawPath(canvas, currentPath, currentColor, currentStrokeWidth, currentTool)
        }
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (currentTool in listOf("rectangle", "circle", "arrow")) {
                    currentPath.clear()
        ...
        when (tool) {
            "highlight" -> {
                if (points.size >= 2) {
                    ...
                    val path = Path()
                    path.moveTo(points[0].x, points[0].y - 10f)
                    for (i in 1 until points.size) {
                        path.lineTo(points[i].x, points[i].y - 10f)
                    }
            ...
            "circle" -> {
                if (points.size >= 2) {
                    ...
                    val radius = kotlin.math.hypot(points[1].x - points[0].x, points[1].y - points[0].y) / 2f
            ...
            "arrow" -> {
                if (points.size >= 2) {
                    ...
                    // Arrow head
                    val angle = kotlin.math.atan2(points[1].y - points[0].y, points[1].x - points[0].x)
                    val arrowLen = 20f
                    val arrowAngle = 0.5f
                    val x1 = points[1].x - arrowLen * kotlin.math.cos(angle - arrowAngle)
                    val y1 = points[1].y - arrowLen * kotlin.math.sin(angle - arrowAngle)
                    val x2 = points[1].x - arrowLen * kotlin.math.cos(angle + arrowAngle)
                    val y2 = points[1].y - arrowLen * kotlin.math.sin(angle + arrowAngle)
                    canvas.drawLine(points[1].x, points[1].y, x1, y1, paint)
                    canvas.drawLine(points[1].x, points[1].y, x2, y2, paint)
                }
            }
            "eraser" -> {
                val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {

...
class AnnotationCanvasView @JvmOverloads constructor(
    ...
    // Get bitmap of just the annotations (transparent background)
    ...
    // NEW: Save annotations overlay to a PNG file
    fun saveAnnotationsToFile(outputFile: File): Boolean {
        return try {
            val bitmap = getAnnotationsBitmap(width, height)
    ...
    // NEW: Merge annotations with background and save as PDF page image
    fun saveAnnotatedPage(backgroundBitmap: Bitmap, outputFile: File): Boolean {
        return try {
            val result = Bitmap.createBitmap(backgroundBitmap.width, backgroundBitmap.height, Bitmap.Config.ARGB_8888)
            ...
            val annotations = getAnnotationsBitmap(width, height)
            canvas.drawBitmap(annotations, 0f, 0f, null)

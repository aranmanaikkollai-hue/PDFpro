...
class MainActivity : AppCompatActivity() {
    ...
    private fun createQuickActionsGrid(): LinearLayout {
        ...
        data class Action(val icon: String, val label: String, val color: Int, val action: () -> Unit)

        val actions = listOf(
            Action("📂", "Open PDF", Color.parseColor("#FF6B35")) { openPdf() },
            Action("✂️", "PDF Tools", Color.parseColor("#004E89")) { startActivity(Intent(this, ToolsActivity::class.java)) },
            Action("📷", "Scanner", Color.parseColor("#1A936F")) { startActivity(Intent(this, DocumentScannerActivity::class.java)) },
            Action("⚙️", "Settings", Color.parseColor("#9B59B6")) { startActivity(Intent(this, SettingsActivity::class.java)) }
        )

        actions.chunked(2).forEach { row ->
            val rowLayout = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
            row.forEach { action ->
                rowLayout.addView(createActionCard(action.icon, action.label, action.color, action.action))
            }
    ...
    private fun createPremiumBanner(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            addView(Button(this@MainActivity).apply {
                ...
                setOnClickListener {
                    Toast.makeText(context, "All Premium Features Unlocked!", Toast.LENGTH_SHORT).show()
                }
            })
        }
    }
    ...
    private fun checkPermissions() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            if (ContextCompat.checkSelfPermission(this, Manifest.permission.READ_MEDIA_IMAGES) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.READ_MEDIA_IMAGES), REQUEST_STORAGE)
            }
        } else {

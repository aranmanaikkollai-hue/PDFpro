...
class MainActivity : AppCompatActivity() {
    ...
    private val expandedCategories = mutableSetOf()
    ...
    private val ocrPicker = registerForActivityResult(ActivityResultContracts.GetContent()) { _: Uri? ->
        toast("OCR from gallery -- use DocumentScannerActivity")
    }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.uiState.collect { state ->
                    ...
                    allFileEntities = state.files
                    if (::sortBtn.isInitialized) sortBtn.text = sortLabel()
                    if (::viewToggleBtn.isInitialized) viewToggleBtn.setImageResource(viewToggleIcon())
                    rebuildFileList()
...
Tap + to open a PDF"; textSize = 14f; gravity = Gravity.CENTER
    ...
    private fun buildTabBar(): LinearLayout {
        ...
        listOf("Recent" to "recent", "Starred" to "starred", "Vault" to "categories", "Bookmarks" to "bookmarks").forEach { (label, id) ->
            ...
            tabRow.addView(TextView(this).apply {
                ...
                setOnClickListener { viewModel.switchTab(id) }
            })
    ...
    private fun buildSortViewRow(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            viewToggleBtn = ImageButton(this@MainActivity).apply {
                ...
                setOnClickListener { viewModel.cycleViewMode() }
            }
    ...
    private fun buildBottomNav(): FrameLayout {
        ...
        listOf(NI("home","HOME",android.R.drawable.ic_menu_view), NI("tools","TOOLS",android.R.drawable.ic_menu_preferences),
            ...
            val active = item.id == "home" && currentTab in listOf("recent","starred","categories","bookmarks")
            val col = LinearLayout(this).apply {
                orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER; layoutParams = LinearLayout.LayoutParams(0, -1, 1f)
                setOnClickListener { when (item.id) { "home" -> if (currentTab != "recent") viewModel.switchTab("recent"); "tools" -> startActivity(Intent(this@MainActivity, ToolsActivity::class.java)); "scan" -> startActivity(Intent(this@MainActivity, DocumentScannerActivity::class.java)); "ocr" -> showOcrDialog() } }
            }
    ...
    private fun updateNavHighlight(tab: String) {
        ...
        listOf("home","tools","scan","ocr").forEach { id ->
            ...
            rootFrame.findViewWithTag("${id}_icon")?.setColorFilter(if (active) c_pri else Color.parseColor(txt2()))
            rootFrame.findViewWithTag("${id}_label")?.setTextColor(if (active) c_pri else Color.parseColor(txt2()))
        }
    }

    private fun toggleTheme() { viewModel.toggleTheme(); thumbnailCache.clear(); buildUI() }
    ...
    private fun showSortMenu() {
        AlertDialog.Builder(this).setTitle("Sort by")
            .setItems(arrayOf("Date (newest)","Date (oldest)","Name A-Z","Name Z-A","Size (large)","Size (small)")) { _, which ->
                when (which) {
                    0 -> viewModel.setSortMode("date", false)
                    1 -> viewModel.setSortMode("date", true)
                    2 -> viewModel.setSortMode("name", false)
                    3 -> viewModel.setSortMode("name", true)
                    4 -> viewModel.setSortMode("size", false)
                    5 -> viewModel.setSortMode("size", true)
                }
            }.show()
    }
...
Tap + to open a PDF" }
    ...
    private fun buildFileCard(f: RecentFileEntity): View {
        ...
        row.addView(ImageButton(this).apply {
            ...
            setOnClickListener { viewModel.toggleFavourite(f.uri) }
        })
    ...
    private fun renderVault() {
        ...
        fileListContainer.addView(TextView(this).apply {
            text = "\u21bb Scan Local Files"; textSize = 12f; typeface = Typeface.DEFAULT_BOLD; setTextColor(c_pri)
            gravity = Gravity.CENTER; setPadding(0, dp(8), 0, dp(8))
            setOnClickListener { viewModel.scanLocalFiles(this@MainActivity); toast("Scanning local PDFs...") }
        })
        ...
        topLevel.forEach { cat ->
            ...
            fileListContainer.addView(buildVaultCard(cat, (fileCats[cat] ?: emptyList()).size, 0))
            if (expandedCategories.contains(cat)) {
                ...
                subFolders.forEach { sub ->
                    ...
                    if (expandedCategories.contains(sub)) {
                        subFiles.sortedByDescending { it.lastOpenedAt }.forEach { f -> fileListContainer.addView(buildFileTileRow(f).also { row -> (row.layoutParams as? LinearLayout.LayoutParams)?.marginStart = dp(40) }) }
                    }
                }
                val directFiles = fileCats[cat] ?: emptyList()
                directFiles.sortedByDescending { it.lastOpenedAt }.forEach { f -> fileListContainer.addView(buildFileTileRow(f).also { row -> (row.layoutParams as? LinearLayout.LayoutParams)?.marginStart = dp(20) }) }
            }
        }
    }
        ...
        AlertDialog.Builder(this).setTitle("Rename folder").setView(et)
            .setPositiveButton("Save") { _, _ ->
                ...
                if (n.isNotEmpty() && n != displayName) {
                    ...
                    viewModel.renameCategory(cat, newFull)
                }
            }.setNegativeButton("Cancel", null).show()
    }
    ...
    private fun showMoveAllDialog(targetCat: String) {
        ...
        AlertDialog.Builder(this).setTitle("Move all files from:").setItems(saved.toTypedArray()) { _, i ->
            val src = saved[i]
            lifecycleScope.launch(Dispatchers.IO) { allFileEntities.filter { it.category == src }.forEach { viewModel.moveToCategory(it.uri, targetCat) }; withContext(Dispatchers.Main) { rebuildFileList(); toast("Moved files to ${targetCat.substringAfterLast("/")}") } }
        }.show()
    }
    ...
    private fun showCreateFolderDialog() {
        ...
        AlertDialog.Builder(this).setTitle("New Folder").setView(et)
            .setPositiveButton("Create") { _, _ ->
                val n = et.text.toString().trim()
                if (n.isNotEmpty()) { viewModel.createCategory(n) }
            }.show()
    }
    ...
    private fun showMoveToCategoryDialog(f: RecentFileEntity) {
        ...
        AlertDialog.Builder(this).setTitle("Move to Folder").setItems(display) { _, i ->
            viewModel.moveToCategory(f.uri, saved[i])
        }.show()
    }
    ...
    private fun showFileOptions(f: RecentFileEntity) {
        ...
        AlertDialog.Builder(this).setTitle(f.displayName).setItems(ops) { _, w ->
            when (w) {
                ...
                1 -> viewModel.toggleFavourite(f.uri)
                ...
                4 -> AlertDialog.Builder(this).setTitle("Delete?").setMessage("Remove \"${f.displayName}\" from recent list?")
                    .setPositiveButton("Remove") { _, _ -> viewModel.deleteFile(f.uri) }
                    .setNegativeButton("Cancel", null).show()
                5 -> shareFile(f)
                6 -> showFileInfo(f)
            }
    ...
    private fun showRenameDialog(f: RecentFileEntity) {
        ...
        AlertDialog.Builder(this).setTitle("Rename").setView(et)
            .setPositiveButton("Save") { _, _ ->
                val n = et.text.toString().trim()
                if (n.isNotEmpty()) { viewModel.renameFile(f.uri, n) }
            }.setNegativeButton("Cancel", null).show()
    }
    ...
    private fun showSearchDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setView(et)
            .setPositiveButton("Find") { _, _ -> viewModel.search(et.text.toString().trim()) }
            .setNegativeButton("Close") { _, _ -> viewModel.search("") }
            .show()
    }

    private fun showSettings() {
        AlertDialog.Builder(this).setTitle("Settings")
            .setItems(arrayOf("Clear Recent Files (keep starred/categorized)", "Delete ALL Files", "Scan Local PDFs", "About")) { _, i ->
                when (i) {
                    0 -> AlertDialog.Builder(this).setTitle("Clear Recent?").setMessage("Removes only recent non-starred and non-bookmarked files.")
                        .setPositiveButton("Clear") { _, _ -> viewModel.clearRecentFiles() }
                        .setNegativeButton("Cancel", null).show()
                    1 -> AlertDialog.Builder(this).setTitle("Delete ALL?").setMessage("This removes every file from the list.")
                        .setPositiveButton("Delete All") { _, _ -> viewModel.clearAllFiles() }
                        .setNegativeButton("Cancel", null).show()
                    2 -> viewModel.scanLocalFiles(this)
                    3 -> AlertDialog.Builder(this).setTitle("ProPDF Editor v4.0").setMessage("Production-grade PDF editor with AI features.\nBuild: Codemagic CI/CD.").setPositiveButton("OK", null).show()
                }
            }.show()
    }
    ...
    private fun openUri(uri: Uri) {
        lifecycleScope.launch {
            if (!isValidOpenUri(uri)) return@launch
    ...
    private fun isValidOpenUri(uri: Uri): Boolean {
        val uriText = uri.toString().trim()
        if (uriText.isEmpty()) { toast("Invalid file URI"); return false }
        if (uri.scheme == "file") {
            val localFile = java.io.File(uri.path ?: "")
            if (!localFile.exists() || !localFile.isFile) { toast("File does not exist"); return false }
            if (!FileHelper.isPdf(localFile)) { toast("Not a valid PDF"); return false }
            return true
        }
        if (!FileHelper.isValidPdfUri(this, uri)) { toast("Cannot open this PDF URI"); return false }
        return true
    }
